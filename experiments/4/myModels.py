...
class LowLightEnhancer(nn.Module):
    def __init__(self, optim_choice = 'Adam', lr = 1e-3, device = torch.device("cpu")):
        ...
        self.decom = DecomNet()
        self.relight_optimizer = optim.SGD(self.relight.parameters(), lr=self.lr, momentum = 0.9, weight_decay = 1e-5)
        self.decom_optimizer = optim.SGD(self.decom.parameters(), lr=self.lr, momentum = 0.9, weight_decay = 1e-5)
    ...
    def train(self, S_low, S_high):
        ...
        self.L_Decom += L_same_R
        
        self.decom_optimizer.zero_grad()
        self.L_Decom.backward()
        self.decom_optimizer.step()

        self.relight.train()
        Ihat = self.relight(R_low.detach(), I_low.detach())
        L_relight = (R_low.detach()*Ihat - S_high).abs().mean()
        self.Ismooth_loss_delta = self.smooth(Ihat, R_low.detach())
        self.L_Relight = L_relight + 3*(self.Ismooth_loss_delta)

        self.relight_optimizer.zero_grad()
        self.L_Relight.backward()
        self.relight_optimizer.step()
    ...
    def gradient(self, input, direction):
        ...
        self.smooth_kernel_y = torch.permute(self.smooth_kernel_x, (1, 0, 2, 3))
        if direction == "x":

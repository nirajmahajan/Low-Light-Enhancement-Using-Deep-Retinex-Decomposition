...
class LowLightEnhancer(nn.Module):
    def __init__(self, optim_choice = 'Adam', lr = 1e-3, device = torch.device("cpu")):
        ...
        self.decom = DecomNet()
        self.disc = models.alexnet(num_classes = 2)
        self.optimizer = optim.Adam(list(self.relight.parameters()) + list(self.decom.parameters()), lr=self.lr, betas=(.5, 0.999))
        self.optimizer_disc = optim.Adam(self.disc.parameters(), lr=self.lr, betas=(.5, 0.999))
    ...
    def train(self, S_low, S_high):
        ...
        self.L_Decom = L_recon_low + L_recon_high
        self.L_Decom += 0.001 * (L_recon_dual_low + L_recon_dual_high)
        self.L_Decom += 0.1 * (self.Ismooth_loss_low + self.Ismooth_loss_high)
        self.L_Decom += 0.01 * L_same_R
        
        self.relight.train()
        L_relight = (R_low.detach()*Ihat - S_high).abs().mean()
        self.L_Relight = L_relight + 3*(self.Ismooth_loss_delta)

        fake_data = R_low*Ihat.detach()
        labels = torch.ones(fake_data.shape[0]).to(self.device).long()
        preds =self.disc(fake_data)
        self.L_disc = self.criterion_disc(preds, labels)

        loss = self.L_Decom + self.L_Relight + 0.001*self.L_disc

        loss.backward()
        self.optimizer.step()


        ## discrimininator training
        self.optimizer_disc.zero_grad()
        fake_data = R_low.detach()*Ihat.detach()
        real_data = S_high
        labels = torch.ones(fake_data.shape[0] + real_data.shape[0]).to(self.device).long()
        labels[:fake_data.shape[0]] = 0
        data = torch.cat((fake_data, real_data), 0)
        preds =self.disc(data)
        loss_disc = self.criterion_disc(preds, labels)
        loss_disc.backward()
        self.optimizer_disc.step()

        return self.L_Decom.item(), self.L_Relight.item(), self.L_disc.item()

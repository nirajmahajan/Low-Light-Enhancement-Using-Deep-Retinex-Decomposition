...
class LowLightEnhancer(nn.Module):
    def __init__(self, optim_choice = 'Adam', lr = 1e-3, device = torch.device("cpu")):
        ...
        self.decom = DecomNet()
        self.relight_optimizer = optim.Adam(self.relight.parameters(), lr=self.lr, betas=(.5, 0.999))
        self.decom_optimizer = optim.Adam(self.decom.parameters(), lr=self.lr, betas=(.5, 0.999))

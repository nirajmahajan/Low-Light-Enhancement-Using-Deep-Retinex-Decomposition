...
def train(e):
    ...
    for batch_num,(S_low, S_high) in tqdm(enumerate(trainloader), desc = 'Epoch {}'.format(e), total = len(trainloader)):
        L_Decom, L_Relight, L_disc = model.train(S_low, S_high)
        tot_loss_decom += L_Decom
        tot_loss_relight += L_Relight
        tot_loss_disc += L_disc
    ...
    print('Total Discriminator Loss for epoch = {}'.format(tot_loss_disc/batch_num), flush = True)
    return tot_loss_decom/batch_num, tot_loss_relight/batch_num, tot_loss_disc/batch_num
